...
<body class="white-bg skin-<%=ServiceUtil.getThemeType(10)%>">
          ...
          <div class="form-inline">
            <div class="form-group clearfix">
              ...
              <button type="button" onclick=" $('#dicList').bootstrapTable('refresh');" class="btn btn-primary">搜索</button> 
                 ...
                 <c:if test="<%=!ServiceUtil.haveExp(\"/backstage/govMemorizer/index\") %>">
  ...
  <div id="layer_form" style="display: none;" class="ibox-content">
    <form method="post" class="form-horizontal" id="eform">
     ...
     <c:set var="simpleFields" value="<%=ServiceUtil.getService(\"SimpleFieldsService\").find(ServiceUtil.buildBean(\"SimpleFields@isDeleted=0&className=GovMemorizer\"),\"list_no\",\"asc\")%>"/>
  ...
  <div id="import_form" style="display: none;" class="ibox-content">
    <form method="post" class="form-horizontal" id="importForm">
      <div class="form-group" style="padding:10px;">
        <div class="alert alert-info">请先下载存储器清单模版，根据模版填写数据。
         <a class="alert-link"  href="${base}/upload/excel/memExcel.xlsx">Excel模版下载</a>.<br /> 如导入数据量大，上传请耐心等待！
        </div>
...
var url = '${base}/backstage/govMemorizer/'; //controller 路径
var inited=0;
...
function doFormatter(value, row, index)
{
	...
	<c:if test="<%=!ServiceUtil.haveEdit(\"/backstage/govMemorizer/index\") %>">
	html+='<button type="button" class="btn btn-white" id="edit"  onclick="editRow(\''+row.idForShow+'\')"><i class="fa fa-pencil"></i>&nbsp;修改</button>';
	</c:if>
	<c:if test="<%=!ServiceUtil.haveDel(\"/backstage/govMemorizer/index\") %>">
