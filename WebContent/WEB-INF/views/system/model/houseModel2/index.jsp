...
<body class="white-bg skin-<%=ServiceUtil.getThemeType(10)%>">
            ...
            <div class="form-group clearfix">
              ...
              <div class=" pull-left" style="margin-left:10px;width:250px;">
                <select id="infoT" name="infoT" data-placeholder=" " class="chosen-select" style="width:350px;" tabindex="4" required>
                  <option value=""></option>
                  <option value="1">登记类</option>
                  <option value="2">资质类</option>
                  <option value="3">监管类</option>
                  <option value="4">处罚类</option>
                </select>
              </div>
              <div class="pull-left" style="margin-left:10px;">
                <button type="button" onclick=" $('#dicList').bootstrapTable('refresh');" class="btn btn-primary">搜索
                </button>
                <c:if test="<%=!ServiceUtil.haveAdd(\"/backstage/model/houseModel/index?types=2\") %>">
  ...
  <div id="layer_form" style="display: none;" class="ibox-content">
    <form method="post" class="form-horizontal" id="eform">
      ...
      <input type="hidden" name="infoName" class="form-control" >
...
$(formId).form({
  url: url + 'insertAjax?houseTypes=2',
  onSubmit: function() {
...
var TableInit = function() {
  ...
  oTableInit.Init = function() {
    $(tableId).bootstrapTable({
      url: '${base}/backstage/model/houseModel/listAjax?houseTypes=2', //请求后台的URL（*）
      method: 'post', //请求方式（*）
      contentType: "application/x-www-form-urlencoded",
...
function doFormatter(value, row, index) {
  ...
  <c:if test="<%=!ServiceUtil.haveEdit(\"/backstage/model/houseModel/index?types=2\") %>">
  html += '<button type="button" class="btn btn-white" onclick="editRow(\'' + row.idForShow + '\')"><i class="fa fa-pencil"></i>&nbsp;修改</button>';
  </c:if>
  <c:if test="<%=!ServiceUtil.haveDel(\"/backstage/model/houseModel/index?types=2\") %>">
