        ...
        <div class="btn-group hidden-xs" id="toolbar" role="group"  style="display: block;" >
          <ul id="deper" class="clearfix">
            ...
            <li class="col-sm-4" >
              ...
              <c:set var="companyIds" scope="session" value="<%= AccountShiroUtil.getCurrentUser().getCompanyId() %>"/> 
              <c:set var="pid" scope="session" value="${item.id}"/>
              <c:set  var="count"   value="<%=ServiceUtil.getService(\"InformationResourceService\").count(ServiceUtil.buildBean(\"InformationResource@status=0&inforTypes2=\"+session.getAttribute(\"pid\")+\"&companyId=\"+session.getAttribute(\"companyIds\")))%>"/>
              <c:if test="${count > 0}">
              <i>
                ${count}
              </i>
              ...
              <li class="col-sm-4">
                ...
                <c:set  var="count2"   value="<%=ServiceUtil.getService(\"InformationResourceService\").count(ServiceUtil.buildBean(\"InformationResource@status=0&inforTypes3=\"+session.getAttribute(\"pid2\")+\"&companyId=\"+session.getAttribute(\"companyIds\")))%>"/>
                <c:if test="${count2 > 0}">
                <i>
                  ${count2}
                </i>
