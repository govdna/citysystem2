  ...
  <div class="wrapper wrapper-content animated fadeInRight">
    <div class="ibox float-e-margins">
      <div class="ibox-content ibox-content-change">
        <div class="btn-group hidden-xs rel" id="toolbar" role="group" style="display: block;">
          <ul id="deper" class="clearfix">
            <c:forEach var="obj" items="<%=ServiceUtil.getDicByDicNum(\"ZYTGF\") %>">
              <li class="col-sm-4">
                <span class="label label-success btn" value="${obj.dicKey}" dt="1" title="${obj.dicValue}">
                <c:set var="pname" scope="session" value="${obj.dicValue}"/>
                ...
                <c:set  var="count"   value="<%=ServiceUtil.getService(\"InformationResService\").count(ServiceUtil.buildBean(\"InformationRes@inforProvider=\"+session.getAttribute(\"pid\")))%>"/>
                <c:if test="${count > 0}">
                <i>
                ${count}
                </i>
          ...
          <div id="deperreturn" class="text-center" style="display: none;">
            <a href='${base}/backstage/information/res/depart'><span class="label" >返回</span></a>  
          </div>
        </div>
  ...
  <div id="layer_form" style="display: none;" class="ibox-content">
    <form method="post" class="form-horizontal eform1" id="eform">
      ...
      <div class="form-group">
        ...
        <div class="col-sm-4">
          <select name="inforTypesForShow" class="chosen-select" data-placeholder=" " required disabled="true"　readOnly="true" >
            <option value=""></option>
            <c:forEach var="obj" items="<%=ServiceUtil.getDicByDicNum(\"ZYTFL\") %>">
            <option value="${obj.id}">${obj.dicValue}</option>
            </c:forEach>
          </select>
        </div>
        <div class="col-sm-4">
          <select name="inforTypes"  class="chosen-select" data-placeholder=" " required disabled="true"　readOnly="true" >
          <c:forEach var="obj" items="<%=ServiceUtil.getDicByNumKey(\"ZYTFL\") %>">
          <option value="${obj.dicKey}">${obj.dicValue}</option>
          </c:forEach>
          </select>
        </div>
