      ...
      <div class="ibox-content ibox-content-change">
        <div class="btn-group hidden-xs" id="toolbar" role="group" style="display: block;">
          <ul id="deper" class="clearfix">
            <c:forEach items="<%=ServiceUtil.getService(\"SortManagerService\").find(ServiceUtil.buildBean(\"SortManager@isDeleted=0&sortId=133&type=3\"))%>" var="item" varStatus="i"> 
              <li class="col-sm-4">
              <c:set var="pname" scope="session" value="${item.sortName}"/>
              <span class="name label label-success fir" value="${item.id}"  dt="1" title="${item.sortName}">
              ...
              <c:set  var="count"   value="<%=ServiceUtil.getService(\"InformationResService\").count(ServiceUtil.buildBean(\"InformationRes@inforTypes2=\"+session.getAttribute(\"pid\")))%>"/>
              <c:if test="${count > 0}">
              <i>
                ${count}
              </i>
              ...
              <c:forEach var="obj" items="<%=ServiceUtil.getService(\"SortManagerService\").find(ServiceUtil.buildBean(\"SortManager@isDeleted=0&sortId=\"+session.getAttribute(\"pid\")))%>" varStatus="j">
                <li class="col-sm-4">
                <c:set var="pname2" scope="session" value="${obj.sortName}"/>
                <span class="label label-success sed" value="${obj.id}" dt="2" title="${obj.sortName}">
                ...
                <c:set  var="count2"   value="<%=ServiceUtil.getService(\"InformationResService\").count(ServiceUtil.buildBean(\"InformationRes@inforTypes3=\"+session.getAttribute(\"pid2\")))%>"/>
                <c:if test="${count2 > 0}">
                <i>
                  ${count2}
                </i>
          ...
          <a href='${base}/backstage/information/res/theme'><span class="label" >返回</span></a>  
          </div>
        </div>
      ...
      <div class="form-group">
        ...
        <div class="col-sm-4">
          <select name="inforTypes" data-placeholder=" "
            ...
            <option value=""></option>
            <c:forEach var="obj"
              items="<%=ServiceUtil.getService(\"SortManagerService\").find(ServiceUtil.buildBean(\"SortManager@isDeleted=0&level=1\"))%>">
