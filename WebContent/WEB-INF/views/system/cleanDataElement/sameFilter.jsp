...
<body class="white-bg skin-<%=ServiceUtil.getThemeType(10)%>">
  ...
  <div class="wrapper wrapper-content animated fadeInRight">
   <div class="ibox float-e-margins">
	...
	<div id="data_list" style="display: none;" class="ibox-content">
...
$('body').on('click','.com-count',function(e){
	...
	layerIndex=layer.open({
	      ...
	      yes : function(index, layero) {
	    	  if($('input[name="checkDEId"]:checked').length>0){
	    		  sameList();
	    	  }else{
	    		  layer.msg('请选择一条以上数据元进行清洗!');
	    	  }
...
function sameList(){
	...
	$.post('${base}/backstage/cleanDataElement/sameList', $("#listForm").serialize(),function(r){
		$('#repeatForm').html(r);
		...
		var ids="";
		$('input[name="checkDEId"]:checked').each(function(i){
			ids+=$(this).val()+",";
		 });
		
		$('input[name="dataElementIds"]').val(ids);
...
var ChildTableInit = function() {
  ...
  oTableInit.Init = function() {
    $('#listTable').bootstrapTable({
      url : '${base}/backstage/cleanDataElement/getListByConfig', //请求后台的URL（*）
      method : 'post', //请求方式（*）
      contentType : "application/x-www-form-urlencoded",
      iconSize : 'outline',
      striped : true, //是否显示行间隔色
...
$("#data_list").on("click",'#allC',function(){
    var isChecked = $(this).prop("checked");
    $("input[name='checkDEId']").prop("checked", isChecked);   
});
...
function doFormatter3(value, row, index)
{
  var html='';
  html+='<div class="btn-group">';
  html+='<input type="checkbox" name="checkDEId" value="'+row.id+'"/> ';
  html+='</div>';
  return html;
}
