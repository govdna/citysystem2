    ...
    <div class="ibox float-e-margins">
      <div class="ibox-content ibox-content-change">
        <div class="btn-group hidden-xs" id="toolbar" role="group"  style="display: block;">
          ...
          <li class="col-sm-4">
          <span class="label label-success" value="${obj.id}" dt="2" title="${obj.companyName}">
          <c:set var="pname" scope="session" value="${obj.companyName}"/>
          <c:set  var="titleDispose"   value="<%=ServiceUtil.titleDispose(session.getAttribute(\"pname\").toString(), 8)%>"/> ${titleDispose}
          <c:set var="pid" scope="session" value="${obj.id}"/>               
          <c:set var="count" value="<%=ServiceUtil.getService(\"InformationResourceService\").count(ServiceUtil.buildBean(\"InformationResource@status=0&value3=\"+session.getAttribute(\"pid\")))%>"/>
          <c:if test="${count > 0}">
          <i>
             ${count}
          </i>
          ...
          <a href='${base}/backstage/information/resource/depart'><span class="label" >返回</span></a>
          </div>
        </div>
